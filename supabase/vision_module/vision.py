...
class VisionModule:
    ...
    def __init__(self):
        ...
        self._skip_n = 1          # process every Nth frame

        # Distance smoothing (EMA)
        self._smoothed_distance: Optional[float] = None
        self._ema_alpha = 0.3  # lower = smoother, higher = more responsive
    ...
    def calibrate(self, bgr_frame: np.ndarray, known_distance_cm: float = 50.0) -> bool:
        ...
        The LIVE_STREAM landmarker is async so its latest result may not yet
        contain the calibration frame.  We therefore run a *synchronous*
        IMAGE-mode detection on the submitted frame so calibration always
        ...
        try:
            ...
            base_opts = mp_python.BaseOptions(model_asset_buffer=model_bytes)
            sync_options = mp_vision.FaceLandmarkerOptions(
                base_options=base_opts,
            ...
            sync_landmarker = mp_vision.FaceLandmarker.create_from_options(sync_options)

            rgb = cv2.cvtColor(bgr_frame, cv2.COLOR_BGR2RGB)
            mp_img = mp.Image(image_format=mp.ImageFormat.SRGB, data=rgb)
            sync_result = sync_landmarker.detect(mp_img)
            sync_landmarker.close()

            if not sync_result.face_landmarks:
                # Fall back to the async result if the sync detection missed
                result = self._get_latest_result()
                if result is None or not result.face_landmarks:
                    return False
                landmarks = result.face_landmarks[0]
            else:
                landmarks = sync_result.face_landmarks[0]

        except Exception as exc:
            print(f"[VisionModule] calibrate sync detection error: {exc}")
            return False
